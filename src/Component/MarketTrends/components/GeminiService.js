...
export const generateCropAnalysis = async (cropName: string): Promise<GeminiResponse> => {
  try {
...
const cacheAnalysisData = (cropName: string, data: GeminiResponse) => {
  try {
...
const sanitizeResponse = (data: any, cropName: string): GeminiResponse => {
...
const createFallbackDataFromText = (text: string, cropName: string): GeminiResponse => {
...
const createFallbackData = (cropName: string): GeminiResponse => {
...
function extractNumber(text: string, contextKeywords: string[]): number | null {
...
function categorizeCrop(cropName: string): 'vegetable' | 'fruit' | 'grain' | 'spice' | 'other' {
...
function generateRandomPrice(cropName: string, monthOffset: number = 0): number {
...
function getCropPrediction(cropName: string, category: string): string {
  const predictions = {
    'vegetable': [
...
function getCropRecommendations(cropName: string, category: string): string[] {
...
function getCropMarketInsights(cropName: string, category: string): string {
  const insights = {
    'vegetable': [
...
export const generateRecommendations = async (cropType: string = 'general'): Promise<string[]> => {
  try {
...
function getFallbackRecommendations(cropType: string): string[] {
